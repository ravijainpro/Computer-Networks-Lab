...
int main() { 
	...
	srand(time(NULL));
	short int Start_SID=300;
	short int Start_SeqNO=0;
	struct packet to_send ={rand()%3, MAX_TTL, Start_SID++, Start_SeqNO++, 30,"197.0.0.3"};	

	struct packet *data = malloc(sizeof(struct packet));
	struct sockaddr_in A_addr, C_addr, B_addr; 
	
	if ( (sockfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0 ) { 
		perror("NODE C: Socket creation failed"); 
		exit(EXIT_FAILURE); 
	} 
	...
	sleep(10);
	if (( bind(sockfd, (const struct sockaddr *)&C_addr, sizeof(C_addr)) < 0 ))
	{ 
		perror("NODE C: Socket connection failed"); 
		exit(EXIT_FAILURE); 
	} 
	int len, n; 
	len = sizeof(C_addr); 
	while(1) {
		...
		printf("NODE C: Message sent.\n"); 
		sleep(5);
		n = recvfrom(sockfd, (struct packet*)data, sizeof(*data), MSG_WAITALL, ( struct sockaddr *) &B_addr, &len); 
...
*/
	...
	data->TTL-=1;
	struct packet green_1 = {2, data->TTL-1, data->SID, data->SeqNo, 30, "197.0.0.3"};
	struct packet green_2 = {2, data->TTL-1, data->SID, data->SeqNo, 30, "197.0.0.3"};
...
*/
	...
	if(data->TTL>0) {
			struct packet green_pack = {2, data->TTL-1, data->SID, data->SeqNo, 30, "197.0.0.3"};
			if(data->UID==20) {
...
*/
	...
	sleep(5);
	struct packet red_pack = {2, 8, data->UID, Start_SeqNO++, 30, "197.0.0.3"};
	if(data->UID==20) {
