...
int main() { 
	...
	srand(time(NULL));
	short int Start_SID=200;
	short int Start_SeqNO=0;
	struct packet to_send ={rand()%3, MAX_TTL, Start_SID++, Start_SeqNO++, 20,"197.0.0.2"};	

	struct packet *data = malloc(sizeof(struct packet));
	struct sockaddr_in A_addr, C_addr, B_addr; 
	
	if ( (sockfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0 ) { 
		perror("NODE B: Socket creation failed"); 
		exit(EXIT_FAILURE); 
	} 
	...
	if (( bind(sockfd, (const struct sockaddr *)&B_addr, sizeof(B_addr)) < 0 ))
	{ 
		perror("NODE B: Socket connection failed"); 
		exit(EXIT_FAILURE); 
	} 
	sleep(10);
	int len, n; 
	...
	while(1) {
		...
		printf("NODE B: Message sent.\n"); 
		sleep(5);		
		n = recvfrom(sockfd, (struct packet *)data, sizeof(*data), MSG_WAITALL, ( struct sockaddr *) &A_addr, &len); 
...
*/
	...
	data->TTL-=1;
	struct packet green_1 = {2, data->TTL-1, data->SID, data->SeqNo, 20, "197.0.0.2"};
	struct packet green_2 = {2, data->TTL-1, data->SID, data->SeqNo, 20, "197.0.0.2"};
	sendto(sockfd, (struct packet*)&green_1, sizeof(green_1), 
		MSG_CONFIRM, (const struct sockaddr *) &C_addr, sizeof(C_addr)); 
	printf("NODE B: Message sent.\n"); 
	sendto(sockfd, (struct packet*)&green_2, sizeof(green_2), 
		MSG_CONFIRM, (const struct sockaddr *) &A_addr, sizeof(A_addr)); 
	printf("NODE B: Message sent.\n"); 
}
/*
...
*/
	...
	if(data->TTL>0) {
			struct packet green_pack = {2, data->TTL-1, data->SID, data->SeqNo, 20, "197.0.0.2"};
			if(data->UID==10) {
...
*/
	...
	sleep(5);
	struct packet red_pack = {2, 8, data->UID, Start_SeqNO++, 20, "197.0.0.2"};
	if(data->UID==10) {
